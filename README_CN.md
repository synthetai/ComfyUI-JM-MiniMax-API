...
此节点检查视频生成任务的状态并等待直到完成。

#### 输入参数：
- **api_key**: MiniMax API 密钥
- **task_id**: 来自视频生成节点的任务 ID
- **check_interval**: 检查间隔（秒）（默认：30，范围：10-300）
- **max_wait_time**: 最大等待时间（秒）（默认：1800/30分钟，范围：5分钟-2小时）

#### 输出：
- **status**: 任务状态（processing, success, failed）
- **file_id**: 生成视频的文件 ID（下载时必需）
- **video_url**: 生成视频的下载 URL（可能为空）
- **cover_image_url**: 视频封面图像的 URL

#### 功能特性：
- **自动轮询**: 持续检查状态直到完成或失败
- **进度跟踪**: 显示已用时间、剩余时间和尝试次数
- **超时保护**: 通过可配置的最大等待时间防止无限等待
- **智能间隔**: 可自定义检查间隔，平衡响应性和 API 使用

### 下载视频节点

此节点使用状态检查的 file_id 下载生成的视频。

#### 输入参数：
- **api_key**: MiniMax API 密钥
- **file_id**: 来自检查视频状态节点的文件 ID
...
#### 处理流程：
1. 使用 file_id 从 MiniMax 文件 API 获取下载 URL
2. 带进度跟踪的视频文件下载
3. 保存到 ComfyUI 输出目录并添加时间戳
...
5. 运行以获取 task_id
6. 使用**检查视频状态**节点监控进度（将自动等待直到完成）
7. 状态为 "success" 后，使用**下载视频**节点和 file_id 保存视频
